...
public class AOPService {
    ...
    @Before("execution(* com.bridgelabz.order.controller.OrderController.*(..))")     //point-cut expression
    public void logBeforeV1(JoinPoint joinPoint) {
        log.info("Initiating API : " + joinPoint.getSignature().getName() + " ");
    }

    //logs method After hitting API
    @After("execution(* com.bridgelabz.order.controller.OrderController.*(..))")     //point-cut expression
    ...
    @Around(value = "execution(* com.bridgelabz.order.controller.OrderController.*(..))")
